...
import { companyInfo, trustedBrands, teamMembers } from '../../constants/siteConfig'
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex -space-x-3">
        {teamMembers.map((member, i) => (
          <img
            ...
            className="w-11 h-11 rounded-full border-2 border-gray-900 object-cover"
      ...
      <div>
        ...
        <p className="text-white/60 text-sm">Dedicated to Your Project's Success</p>
      </div>
    </div>
  )
}
...
export default function Hero({ bgImage = companyInfo.heroBgImage }) {
  ...
  return (
    ...
    >
      {/* Dark gradient — heavy on left (text), lighter on right (shows site activity) */}
      <div className="absolute inset-0 bg-linear-to-r from-gray-950/95 via-gray-950/70 to-gray-900/20" />
            ...
            <div className="space-y-8">
              ...
              <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-5 py-2.5">
                ...
                <Link
                  ...
                  className="text-white/80 text-sm font-medium hover:text-orange-400 transition-colors"
                >
              ...
              <div className="space-y-3">
                <p className="text-orange-400 text-base font-semibold uppercase tracking-widest">
              ...
              <div className="flex flex-wrap gap-3">
                {['Residential Builds', 'Commercial Projects', 'Roads & Infrastructure', 'Renovations'].map((tag) => (
                  <span
                    key={tag}
                    className="text-sm font-medium text-white/80 bg-white/10 border border-white/15 rounded-full px-4 py-1.5"
                  >
                    ✓ {tag}
                  </span>
                ))}
              ...
              <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-xl">
                <div className="flex-1 flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-5 py-4">
                  ...
                  <input
                    ...
                    className="bg-transparent text-white placeholder-white/40 text-base flex-1 outline-none"
                  />
                </div>
                <button
                  type="submit"
                  className="bg-orange-500 text-white font-bold text-base px-7 py-4 rounded-full hover:bg-orange-400 transition-colors whitespace-nowrap shadow-lg shadow-orange-500/30"
                >
